...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Functions for listening and changing GUI-components %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function saveFile(obj, event) 
        ...
        if(file ~= 0)     
            startToCalculate;
            namev = fullfile(path, file); 
            tempFile = fopen(namev, 'w');  
            ...
            if(selection == 2) 
                str = ['<me:PTpair units="Torr" P="', get(editp2, 'String'), '" T="', get(editT2, 'String'), '" startTime="', num2str(xmin(1)), '" excessReactantConc="', get(editNreactant, 'String'), '" timeUnits="sec" precision="dd" default="true" bathGas="N2">\n'];
            else 
                str = ['<me:PTpair units="Torr" P="', get(editp2, 'String'), '" T="', get(editT2, 'String'), '" startTime="', num2str(xmin(1)), '" excessReactantConc="', get(editNreactant, 'String'), '" timeUnits="sec" precision="dd" default="true" bathGas="He">\n'];
            end              
            fprintf(tempFile, str); 
            
            makeFit;
            BACKGROUND = str2double(get(editBackgroundAverage, 'String'));
            selection = get(selectFittingFunction, 'Value');
            if(selection == 6) 
               wall1 = get(editFirstWallRate, 'String');
               wall2 = get(editSecondWallRate, 'String');
            else 
               wall1 = get(editFirstWallRate, 'String');
               wall2 = wall1;
            end
                
            str = ['<me:rawData ref="RADICAL" name="', num2str(BACKGROUND), '" weight="', num2str(weight), '">\n<me:diffusiveLossArray>\n<me:diffusiveLoss speciesRef="RADICAL" loss="', wall1, '"/>\n<me:diffusiveLoss speciesRef="Int1" loss="', wall2, '"/>\n</me:diffusiveLossArray>\n'];         
            fprintf(tempFile, str);
            ...
            for i = 1:size(theData,1)
                ...
                str = [str, ' ', num2str(theData(i,1), '%.6f')];
            end 
            str = [str, ' ', '</me:times>\n'];
            fprintf(tempFile, str); 
            str = '<me:signals>';
            for i = 1:size(theData,1)
                ...
                str = [str, ' ', num2str(theData(i,2) - BACKGROUND,'%.6f' )];
                 %str = [str, ' ', num2str(theData(i,2),'%.6f' )];         
            end 
            str = [str, ' ', '</me:signals>\n'];
            fprintf(tempFile, str);
            str = '</me:rawData>\n</me:PTpair>';         
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Functions for graphing %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function plotFittedExponential(anInterval, params) 
        ...
        weight = ((max(y1)-params(1))/std(residualData(:,2),0)).^2;
